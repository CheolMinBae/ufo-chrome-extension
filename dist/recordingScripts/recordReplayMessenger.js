class RecordReplayMessenger {
    ...
    constructor(options) {
        ...
        const defaults = {
            ...
            windowOnMessageObservable: Rx.Observable.fromEventPattern(
                (handler) => {
                    ...
                    this.logWithContext(
                        `Record/Replay Messenger: SUBSCRIBED to Window Messaging Observer Instance`,
                    );
                },
                (handler) => {
        ...
        //then report
        this.logWithContext(
            `Record/Replay Messenger created in origin ${window.origin}`,
        );
    }
    ...
    sendMessage = (message) =>
        chrome.runtime.sendMessage(message, (response) =>
            this.logWithContext(
                `Record/Replay Messenger: Received Response Message: ${response.message}`,
            ),
        );
