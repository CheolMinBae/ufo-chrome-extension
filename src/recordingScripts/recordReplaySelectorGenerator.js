    ...
    window.recordReplaySelectorGenerator = (element) => {
        ...
        getSelector = function (currentElement) {
            ...
            selectedArray.unshift(
                (baseNodeDescriptor = currentElement.nodeName.toLowerCase()),
            );
            ...
            if (currentElement.className) {
                ...
                classNameArray = classNameArray.filter((item) =>
                    validClassName.test(item),
                );
                //if there is a remaining class name then we add all the classes with a leading dot
                selectedArray[0] = baseNodeDescriptor +=
            ...
            for (let i = 0; i < attributesArray.length; ++i) {
                ...
                if (attributesArray[i] === "data-*") {
                    ...
                    var dataAttributesArray = Array.prototype.slice
                        .call(currentElement.attributes)
                    ...
                    dataAttributesArray = dataAttributesArray.filter(
                        (attr) =>
                            attr.name !== "data-reactid" &&
                            attr.name !== "data-react-checksum",
                    );
                    ...
                    for (let j = 0; j < dataAttributesArray.length; ++j) {
                        ...
                        selectedArray[0] =
                            baseNodeDescriptor += `[${dataAttributesArray[j].name}="${dataAttributesArray[j].value}"]`;
                        //then we test to see if that gives us a unique value and, if so, we return true - we're done
                        if (uniqueQuery()) return true;
                    }
                    ...
                    selectedArray[0] = baseNodeDescriptor += `[${
                        attributesArray[i]
                    }="${currentElement.getAttribute(attributesArray[i])}"]`;
                    //then we test to see if that gives us a unique value and, if so, we return true - we're done
                    if (uniqueQuery()) return true;
                }
            }
            ...
            while ((elementChild = elementChild.previousElementSibling)) {
                if (elementChild.nodeName === currentElement.nodeName) ++count;
            }
            ...
            selectedArray[0] =
                count === 1
                    ? `${baseNodeDescriptor}:first-child`
                    : `${baseNodeDescriptor}:nth-child(${count})`;
            //then we test
            if (uniqueQuery()) return true;
