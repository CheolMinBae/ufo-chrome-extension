...
var EventRecorder = {
    ...
    getCssSelectorPath: (element) =>
        EventRecorder.cssSelectorClass.getSelector(element),
    //then we get a function that returns the CSS optimal selector - again we relegate id here to last place
    getOptimalSelectPath: (element) =>
        OptimalSelect.select(element, {
            priority: ["class", "href", "src", "id"],
        }),
    ...
    getCssSelectorPathWithFailover: (element) =>
        EventRecorder.getCssSelectorPath(element) ||
        EventRecorder.getCssDomPath(element),
    getOptimalSelectPathWithFailover: (element) =>
        EventRecorder.getOptimalSelectPath(element) ||
        EventRecorder.getCssDomPath(element),
    getRecordReplayPathWithFailover: (element) =>
        EventRecorder.getRecordReplayPath(element) ||
        EventRecorder.getCssDomPath(element),
...
EventRecorder.startRecordingEvents = () => {
    ...
    const cssSelectorTimed = EventRecorder.timed(
        EventRecorder.getCssSelectorPathWithFailover,
    );
    const optimalSelectTimed = EventRecorder.timed(
        EventRecorder.getOptimalSelectPathWithFailover,
    );
    const recordReplayTimed = EventRecorder.timed(
        EventRecorder.getRecordReplayPathWithFailover,
    );
    ...
    EventRecorder.MouseLocator = Rx.Observable.merge(
        ...EventRecorder.mouseLocationEventObervables,
    )
        ...
        .throttleTime(250, Rx.Scheduler.async, {
            leading: true,
            trailing: true,
        })
    ...
    EventRecorder.InputLocator = Rx.Observable.merge(
        ...EventRecorder.inputLocationEventObservables,
    )
        ...
        .map((event) => {
            return {
                eventCssSelectorPath:
                    EventRecorder.getCssSelectorPathWithFailover(event.target),
                eventCssOptimalPath:
                    EventRecorder.getOptimalSelectPathWithFailover(
                        event.target,
                    ),
                eventRecordReplayPath:
                    EventRecorder.getRecordReplayPathWithFailover(event.target),
    ...
    EventRecorder.mouseActionObservable = Rx.Observable.merge(
        ...EventRecorder.mouseActionEventObervables,
    )
        ...
        .throttleTime(500, Rx.Scheduler.async, {
            leading: true,
            trailing: true,
        })
        ...
        .switchMap(
            () =>
                Rx.Observable.combineLatest(
                    ...
                    Rx.Observable.merge(
                        ...EventRecorder.attentionActionEventObservables,
                    )
            ...
            (
                [mouseDownEvent, mouseLocatorEvent],
                [clickOrContextMenuEvent, doubleClickEvent, selectStartEvent],
            ) => {
                //then we need to know if we have a valid select start event by timestamp and value
                const selectStartValid =
                    selectStartEvent.timeStamp > mouseDownEvent.timeStamp &&
                    selectStartEvent.type != null;
                //then we need to know if we have a valid double click event by timestamp and value
                const doubleClickValid =
                    doubleClickEvent.timeStamp > mouseDownEvent.timeStamp &&
                    doubleClickEvent.type != null;
                //then we need to know if we have a valid click or contextmenu event
                const clickValid =
                    clickOrContextMenuEvent.timeStamp >
                ...
                switch (true) {
                    ...
                    case selectStartValid && !doubleClickValid:
                        ...
                        return Object.assign({}, mouseLocatorEvent, {
                            type: selectStartEvent.type,
                        });
                    ...
                    default:
                        return Object.assign({}, mouseLocatorEvent, {
                            type: clickOrContextMenuEvent.type,
                        });
                }
            },
        )
        ...
        .do((event) =>
            console.log(`RECORDING MOUSE EVENT: ${event.type.toUpperCase()}`),
        )
    ...
    EventRecorder.textSelectionObservable = EventRecorder.mouseActionObservable
        ...
        .filter(
            (selectWithLocationEvent) =>
                selectWithLocationEvent.type == "selectstart",
        )
        ...
        .map((selectWithLocationEvent) => {
            ...
            return Object.assign({}, selectWithLocationEvent, {
        ...
        .map((selectEndObject) => {
            ...
            if (
                selectEndObject.selectionString ==
                selectEndObject.recordReplayEventTarget.textContent
            ) {
                ...
                selectionString =
                    selectEndObject.recordReplayEventTarget.textContent.trim();
            }
            ...
            const newEvent = new RecordingEvent({
                ...
                recordingEventHTMLElement:
                    selectEndObject.recordReplayEventTarget.constructor.name,
                recordingEventHTMLTag:
                    selectEndObject.recordReplayEventTarget.tagName,
                recordingEventCssSelectorPath:
                    selectEndObject.eventCssSelectorPath,
                recordingEventCssDomPath: selectEndObject.eventCssOptimalPath,
                recordingEventCssFinderPath:
                    selectEndObject.eventRecordReplayPath,
                ...
                recordingEventTextSelectTargetAsJSON: EventRecorder.domToJSON(
                    selectEndObject.recordReplayEventTarget,
                ),
            });
            return newEvent;
        });
    ...
    EventRecorder.mouseObservable = EventRecorder.mouseActionObservable
        ...
        .map((actionWithLocationEvent) => {
            ...
            const newEvent = new RecordingEvent({
                ...
                recordingEventHTMLElement:
                    actionWithLocationEvent.recordReplayEventTarget.constructor
                        .name,
                recordingEventHTMLTag:
                    actionWithLocationEvent.recordReplayEventTarget.tagName,
                recordingEventCssSelectorPath:
                    actionWithLocationEvent.eventCssSelectorPath,
                recordingEventCssDomPath:
                    actionWithLocationEvent.eventCssOptimalPath,
                recordingEventCssFinderPath:
                    actionWithLocationEvent.eventRecordReplayPath,
    ...
    EventRecorder.mouseHoverObservable = EventRecorder.MouseLocator
        ...
        .sample(
            Rx.Observable.merge(
                ...EventRecorder.mouseLocationEventObervables,
            ).filter((event) => event.type == "mouseout"),
        )
        ...
        .filter(
            (timeStampedLayeredObject) =>
                timeStampedLayeredObject.timestamp -
                    timeStampedLayeredObject.value.timestamp >
                3000,
        )
        ...
        .map((mouseLocatorEvent) => {
            const newEvent = new RecordingEvent({
                ...
                recordingEventHTMLElement:
                    mouseLocatorEvent.recordReplayEventTarget.constructor.name,
                recordingEventHTMLTag:
                    mouseLocatorEvent.recordReplayEventTarget.tagName,
                recordingEventCssSelectorPath:
                    mouseLocatorEvent.eventCssSelectorPath,
                recordingEventCssDomPath: mouseLocatorEvent.eventCssOptimalPath,
                recordingEventCssFinderPath:
                    mouseLocatorEvent.eventRecordReplayPath,
                ...
                recordingEventIframeName: EventRecorder.contextIsIframe()
                    ...
                    : "N/A",
                //information specific to text select events
                recordingEventHoverTargetAsJSON: EventRecorder.domToJSON(
                    mouseLocatorEvent.recordReplayEventTarget,
                ),
            });
            return newEvent;
        });
    ...
    EventRecorder.mouseRecaptchaObservable = Rx.Observable.merge(
        ...EventRecorder.mouseActionEventObervables,
    )
        ...
        .map(([actionEvent, locationEvent]) => {
            ...
            const newEvent = new RecordingEvent({
                ...
                recordingEventCssSelectorPath:
                    locationEvent.eventCssSelectorPath,
                recordingEventCssDomPath: locationEvent.eventCssOptimalPath,
                recordingEventCssFinderPath:
                    locationEvent.eventRecordReplayPath,
    ...
    EventRecorder.inputObservable = Rx.Observable.merge(
        ...
        Rx.Observable.fromEvent(document, "blur", true).filter(
            (event) => event.target.isContentEditable,
        ),
    )
        //then as each action occurs, we want to know the state of the element BEFORE the action took place
    ...
    //KEYBOARD EVENTS
    EventRecorder.keyboardObservable = Rx.Observable.merge(
        ...EventRecorder.keyBoardActionEventObservables,
    )
        ...
        .map(([actionEvent, keydownEvent]) => {
            ...
            const keyInfoObject =
                EventRecorder.keyCodeDictionary[actionEvent.keyCode];
            ...
            const newEvent = new RecordingEvent({
                ...
                recordingEventXPath: EventRecorder.getXPath(
                    keydownEvent.target,
                ),
                recordingEventLocation: window.location.origin,
                recordingEventLocationHref: window.location.href,
                ...
                recordingEventDispatchKeyEvent:
                    EventRecorder.keyCodeDictionary.generateDispatchKeyEvent(
                        actionEvent,
                    ),
            });
            return newEvent;
    ...
    //SCROLL EVENTS
    EventRecorder.scrollObservable = Rx.Observable.merge(
        ...EventRecorder.scrollActionEventObservables,
    )
        ...
        .map((actionEvent) => {
            const newEvent = new RecordingEvent({
                ...
                recordingEventActionType: `x: ${Math.round(
                    ...
                    actionEvent.target.scrollingElement.scrollTop,
                )}`,
                recordingEventHTMLElement: actionEvent.target.constructor.name,
                recordingEventHTMLTag:
                    actionEvent.target.scrollingElement.tagName,
                recordingEventCssSelectorPath:
                    EventRecorder.getCssSelectorPathWithFailover(
                ...
                recordingEventXPosition: Math.round(
                    actionEvent.target.scrollingElement.scrollLeft,
                ),
                recordingEventYPosition: Math.round(
                    actionEvent.target.scrollingElement.scrollTop,
                ),
            });
            return newEvent;
    ...
    //SCROLL ITEM EVENTS
    EventRecorder.scrollItemObservable = Rx.Observable.merge(
        ...EventRecorder.scrollActionEventObservables,
    )
        //then we are interested only in scroll events
        .filter((event) => event.type == "scroll")
        ...
        .map((actionEvent) => {
            const newEvent = new RecordingEvent({
                ...
                recordingEventActionType: `x: ${Math.round(
                    actionEvent.target.scrollLeft,
                )}, y: ${Math.round(actionEvent.target.scrollTop)}`,
                recordingEventHTMLElement: actionEvent.target.constructor.name,
                recordingEventHTMLTag: actionEvent.target.tagName,
                recordingEventCssSelectorPath:
                    EventRecorder.getCssSelectorPathWithFailover(
    ...
    //FILE DROP EVENTS
    EventRecorder.fileDropObservable = Rx.Observable.merge(
        ...EventRecorder.fileDropActionEventObservables,
    )
        ...
        .flatMap(([dropEvent, locationEvent]) => {
            ...
            return (async () => {
                ...
                ["image", "video", "audio", "pdf", "octet-stream"].some(
                    (item) => file.type.includes(item),
                )
                ...
                const newEvent = new RecordingEvent({
                    ...
                    recordingEventHTMLElement:
                        locationEvent.recordReplayEventTarget.constructor.name,
                    recordingEventHTMLTag:
                        locationEvent.recordReplayEventTarget.tagName,
                    recordingEventCssSelectorPath:
                        EventRecorder.getCssSelectorPathWithFailover(
